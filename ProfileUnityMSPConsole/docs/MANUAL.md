...
**Running more than one instance side by side** (e.g. a production
install plus a separate demo copy — see README.md's "Demo data" section):
put each copy
...
attachment), in a white band directly beneath Liquidware's own logo and
title, which is unchanged. That band is white specifically so an
uploaded logo shows its true colors rather than being tinted by sitting
on blue; a wide, landscape shape (roughly 4:1, like a wordmark) fits it
better than a square or tall logo. **Remove Logo** clears a previously
