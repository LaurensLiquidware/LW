...
func TestResolveLogFilePath_DefaultsWhenNoEnvFile(t *testing.T) {
	dir := t.TempDir()
	os.Unsetenv("PUMC_LOG_FILE")

	got := resolveLogFilePath(dir)
	want := filepath.Join(dir, "profileunity-msp-console.log")
	if got != want {
...
func TestResolveLogFilePath_HonorsEnvFileOverride(t *testing.T) {
	...
	if err := os.WriteFile(envPath, []byte("PUMC_LOG_FILE=custom.log\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestResolveLogFilePath_AbsoluteOverrideUsedAsIs(t *testing.T) {
	...
	envPath := filepath.Join(dir, ".env")
	if err := os.WriteFile(envPath, []byte("PUMC_LOG_FILE="+abs+"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
