...
func TestResolveServerURL_DefaultsToLocalhost(t *testing.T) {
	os.Unsetenv("FVS_HTTP_ADDR")
	dir := t.TempDir()

	got := resolveServerURL(dir)
	want := "http://127.0.0.1:8743"
	if got != want {
...
func TestResolveServerURL_HonorsEnvFileOverride(t *testing.T) {
	...
	if err := os.WriteFile(envPath, []byte("FVS_HTTP_ADDR=192.168.1.50:9443\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	got := resolveServerURL(dir)
	want := "http://192.168.1.50:9443"
	if got != want {
		t.Errorf("resolveServerURL = %q, want %q", got, want)
	}
	os.Unsetenv("FVS_HTTP_ADDR")
...
func TestResolveServerURL_AllInterfacesBecomesLocalhost(t *testing.T) {
	os.Setenv("FVS_HTTP_ADDR", "0.0.0.0:8443")
	defer os.Unsetenv("FVS_HTTP_ADDR")
	dir := t.TempDir()
...
func TestCurrentHTTPAddr_DefaultsWhenNoEnvOrFile(t *testing.T) {
	...
	if got != "127.0.0.1:8743" {
		t.Errorf("currentHTTPAddr = %q, want %q", got, "127.0.0.1:8743")
	}
}

func TestCurrentHTTPAddr_RealEnvVarWins(t *testing.T) {
	os.Setenv("FVS_HTTP_ADDR", "127.0.0.1:9000")
	defer os.Unsetenv("FVS_HTTP_ADDR")
	dir := t.TempDir()
...
func TestCurrentHTTPAddr_ReadsFileDirectly(t *testing.T) {
	...
	dir := t.TempDir()
	envPath := filepath.Join(dir, ".env")
	if err := os.WriteFile(envPath, []byte("FVS_HTTP_ADDR=0.0.0.0:8444\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestCurrentHTTPAddr_ReflectsFileChangesAcrossCalls(t *testing.T) {
	...
	dir := t.TempDir()
	envPath := filepath.Join(dir, ".env")
	if err := os.WriteFile(envPath, []byte("FVS_HTTP_ADDR=0.0.0.0:8443\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if err := dotenv.SetValue(envPath, "FVS_HTTP_ADDR", "0.0.0.0:8444"); err != nil {
		t.Fatal(err)
	}
