...
- **All five build-order steps from PLAN.md are now complete, and both
  ...
  end** against a real OBS Studio package on a real Windows host - classic
  VHDX and FlexApp One both produced byte-identical results (2170 files,
  zero crashes), and after both bugs above were fixed, a full re-run
  produced a schema-valid inventory JSON, a valid 14-component
  `sbom.cdx.json`, and `coverage-report.md` stating **53.0% resolution
  coverage** - matching the number predicted from the raw JSON, now
  generated by the actual pipeline. What remains is network access for
  live OSV.dev/NVD queries in the `resolve` command, which this dev
  environment still can't provide.
